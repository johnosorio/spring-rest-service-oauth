 ...
 */
package com.proycomp.rest.auth.service.tokenstore;
...
public class MyJdbcTokenStore implements TokenStore {
	...
	public OAuth2AccessToken getAccessToken(OAuth2Authentication authentication) {
		...
		String key = authenticationKeyGenerator.extractKey(authentication);
                System.out.println("JJOC Key: " + key);
		try {
			accessToken = jdbcTemplate.queryForObject(selectAccessTokenFromAuthenticationSql,
					new RowMapper<OAuth2AccessToken>() {
                        ...
                        System.out.println("JJOC AccessKey: " + accessToken.getTokenType() + accessToken.getValue());
		}
		catch (EmptyResultDataAccessException e) {
			if (LOG.isDebugEnabled()) {
				LOG.debug("Failed to find access token for authentication 1" + authentication);
			}
		}
	...
	public void storeAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication) {
		...
		if (readAccessToken(token.getValue())!=null) {
			removeAccessToken(token.getValue());
                ...
                /*if (readAccessToken(authenticationKeyGenerator.extractKey(authentication))!=null) {
			removeAccessToken(token.getValue());
		}*/
	...
	public OAuth2AccessToken readAccessToken(String tokenValue) {
		OAuth2AccessToken accessToken = null;
                ...
                System.out.println("JJOC readAccessToken query: " + selectAccessTokenSql);
                System.out.println("JJOC readAccessToken tokenValue: " + tokenValue);
                System.out.println("JJOC readAccessToken extractTokenKey tokenValue: " + extractTokenKey(tokenValue));
		try {
			accessToken = jdbcTemplate.queryForObject(selectAccessTokenSql, new RowMapper<OAuth2AccessToken>() {
				public OAuth2AccessToken mapRow(ResultSet rs, int rowNum) throws SQLException {
                                        System.out.println("JJOC TokenValue Bytes: " + rs.getBytes(2));
					return deserializeAccessToken(rs.getBytes(2));
				}
			...
			if (LOG.isInfoEnabled()) {
				LOG.info("Failed to find access token for token 2: " + tokenValue + e.getMessage());
			}
		}
		catch (IllegalArgumentException e) {
	...
	public OAuth2Authentication readAuthentication(String token) {
		...
		try {
			...
			if (LOG.isInfoEnabled()) {
				LOG.info("Failed to find access token for token 3" + token);
			}
		}
		catch (IllegalArgumentException e) {
	...
	public OAuth2Authentication readAuthenticationForRefreshToken(String value) {
		...
		try {
			...
			if (LOG.isInfoEnabled()) {
				LOG.info("Failed to find access token for token 4" + value);
			}
		}
		catch (IllegalArgumentException e) {
	...
	public Collection<OAuth2AccessToken> findTokensByClientId(String clientId) {
		...
		try {
			...
			if (LOG.isInfoEnabled()) {
				LOG.info("Failed to find access token for clientId 5" + clientId);
			}
		}
	...
	public Collection<OAuth2AccessToken> findTokensByUserName(String userName) {
		...
		try {
			...
			if (LOG.isInfoEnabled())
				LOG.info("Failed to find access token for userName 6" + userName);
		}
		accessTokens = removeNulls(accessTokens);
	...
	public Collection<OAuth2AccessToken> findTokensByClientIdAndUserName(String clientId, String userName) {
		...
		try {
			...
			if (LOG.isInfoEnabled()) {
				LOG.info("Failed to find access token for clientId 7" + clientId + " and userName " + userName);
			}
		}
		accessTokens = removeNulls(accessTokens);
